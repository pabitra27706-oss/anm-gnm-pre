 ...
 *
 * UPDATED:
 * - All 6 subject pills now always visible.
 * - Subjects with zero questions are shown with a grayed-out style and "(0)" label.
 * - Breakdown shows 0 for empty subjects.
 * - Placeholders removed; empty detection uses allQuestions.emptySubjects array.
 */

(function(window) {
  'use strict';

  var allQuestions = [];        // the question array (with .emptySubjects attached)
  var emptySubjects = [];       // convenience copy
  ...
  function createPill(label, value, group, color, extraClass) {
  ...
  function renderSubjectFilters() {
    ...
    var subjects = FilterEngine.getAllSubjects(); // all 6
    for (var i = 0; i < subjects.length; i++) {
      var s = subjects[i];
      var isEmpty = emptySubjects.indexOf(s) !== -1;
      var label = FilterEngine.getSubjectBn(s);
      if (isEmpty) {
        label += ' (0)';
      }
      var pill = createPill(
        ...
        isEmpty ? 'filter-pill--empty' : ''
      );
      // If empty, mark it as disabled? We'll just let it be selectable but visually indicate zero.
  ...
  function renderUnitFilters() {
    ...
    var unitsBySubject = FilterEngine.getAvailableUnits(
      allQuestions.filter(function(q) { return emptySubjects.indexOf(q._subject) === -1; }),
    ...
    for (var s = 0; s < subjectKeys.length; s++) {
      ...
      for (var u = 0; u < units.length; u++) {
        (function(unitVal, subjectVal) {
          ...
          pill.addEventListener('click', function() { handleUnitChange(unitVal); });
          container.appendChild(pill);
        })(units[u], subject);
      }
    }
  ...
  function updateUnitPillVisuals() {
    ...
    for (var i = 0; i < pills.length; i++) {
      var pill = pills[i];
      var val = pill.dataset.value;
      var isActive = selected.indexOf(val) > -1 || (selected.indexOf('all') > -1 && val === 'all');
      if (isActive) {
        pill.classList.add('active');
        if (val === 'all') {
          pill.style.backgroundColor = '#e91e63';
  ...
  function updateHistoryStats() {
    ...
    if (statsContainer) {
      ...
      if (typeof statusCounts.partial === 'number' && statusCounts.partial > 0) {
        items.push({ label: 'আংশিক', count: statusCounts.partial, cls: 'partial' });
      }
      for (var i = 0; i < items.length; i++) {
        var item = items[i];
        var div  = document.createElement('div');
        div.className = 'history-stat-item';
        div.innerHTML = '<span class="history-dot ' + item.cls + '"></span>' +
    ...
    if (bannerEl) {
      if (overallStats.totalSolved > 0) {
        ...
        if (bannerTitle) bannerTitle.textContent = 'আপনি ' + toBengali(overallStats.totalSolved) + 'টি প্রশ্ন সম্পন্ন করেছেন';
        if (bannerSub) {
          var totalAvailable = allQuestions.filter(function(q) { return emptySubjects.indexOf(q._subject) === -1; }).length;
          var pct = totalAvailable > 0 ? Math.round((overallStats.everCorrect / totalAvailable) * 100) : 0;
          bannerSub.textContent = toBengali(overallStats.everCorrect) + 'টি সঠিক • ' + toBengali(pct) + '% সাফল্যের হার';
        }
        if (progressFill && allQuestions.length > 0) {
          var realCount = allQuestions.filter(function(q) { return emptySubjects.indexOf(q._subject) === -1; }).length;
          var fillPct = realCount > 0 ? Math.round((overallStats.totalSolved / realCount) * 100) : 0;
  ...
  function getBaseFilteredQuestions() {
    ...
    // Exclude empty-subject questions (none, since placeholders removed)
  ...
  function updatePreview() {
    ...
    var quizCount  = currentFilters.count === 'all' ? matchCount : Math.min(matchCount, parseInt(currentFilters.count, 10) || 20);
    ...
    if (breakdownEl) {
      ...
      if (matchCount === 0) {
        ...
        // Include empty subjects with 0 count
        var selectedSubjects = currentFilters.subjects.includes('all') ?
          FilterEngine.getAllSubjects() : currentFilters.subjects;
        for (var i = 0; i < selectedSubjects.length; i++) {
          var sub = selectedSubjects[i];
          if (!breakdown[sub]) {
            breakdown[sub] = { count: 0, multiCount: 0 };
          }
        }
        var subKeys = Object.keys(breakdown);
        for (var j = 0; j < subKeys.length; j++) {
          ...
          if (count === 0 && emptySubjects.indexOf(sub) !== -1) {
            // gray out empty subject
          ...
          var pillText = FilterEngine.getSubjectBn(sub) + ': ' + toBengali(count);
          if (multiCount > 0) {
        ...
        var totalMulti = filtered.filter(function(q) { return q && q.multi === true; }).length;
        if (totalMulti > 0) {
          var multiPill = document.createElement('span');
          multiPill.className = 'breakdown-pill breakdown-pill--multi';
          multiPill.textContent = '☑ মোট বহু-সঠিক: ' + toBengali(totalMulti);
          breakdownEl.appendChild(multiPill);
        }
      }
    }
    console.log('[FilterApp] Preview → match=' + matchCount + ', quiz=' + quizCount);
  }
  ...
  function updateStatsBar() {
    ...
    var realQuestions = allQuestions.filter(function(q) {
      return emptySubjects.indexOf(q._subject) === -1;
    });
    var totalMulti = 0;
    for (var i = 0; i < realQuestions.length; i++) {
      if (realQuestions[i].multi === true) totalMulti++;
    }

    var text = 'মোট ' + toBengali(realQuestions.length) + 'টি প্রশ্ন • ৬টি বিষয়';
    if (totalMulti > 0) {
      text += ' • ' + toBengali(totalMulti) + 'টি বহু-সঠিক';
    }
    if (emptySubjects.length > 0) {
      text += ' (⚠️ ' + emptySubjects.map(function(s) { return FilterEngine.getSubjectBn(s); }).join(', ') + ' - 0)';
    }
  ...
  async function initFilterApp() {
    ...
    // Grab empty subjects list
    emptySubjects = allQuestions.emptySubjects || [];
    console.log('[FilterApp] Questions loaded =', allQuestions.length, '| empty:', emptySubjects.join(', '));
